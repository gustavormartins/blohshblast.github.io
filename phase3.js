...
.phase3-actions{display:flex;gap:8px;margin-top:11px}
  ...
  function installEngineBridge() {
    window.BlohshBlastBridge = {
      getRack: () => {
        const mode = P3.MODE.S;
        if (!mode.smartRng) {
          return [getRandomShapeLocal(), getRandomShapeLocal(), getRandomShapeLocal()];
        }
        return smartRackLocal();
      },
      scoreMultiplier: () => P3.MODE.S.scoreMultiplier,
      onScore: awarded => {
        P3.missionProxy('score', awarded);
        addXP(Math.max(1, Math.round((awarded / 12) * P3.MODE.S.xpMultiplier)));
      },
      onPiecePlaced: amount => P3.missionProxy('pieces', amount),
      onLinesCleared: amount => {
        P3.missionProxy('lines', amount);
        P3.missionProxy('combo', combo);
      },
      onPerfectClear: () => P3.missionProxy('perfect', 1),
      onGameOver: finalScore => {
        if (!finalScore || finalScore <= 0 || P3.gameEnded) return;
        P3.gameEnded = true;
        recordScore(finalScore);
      },
      applySkin: applyPhase3Skin
    };
  }
